...
export function EditContentModal({
  ...
  const [formData, setFormData] = useState({
    ...
    lesson_type: initialData?.lesson_type || "explanation",
  });
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        title: initialData.title || initialData.judul_latihan || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("EditContentModal - Saving data:", dataToSave);
      console.log("EditContentModal - Type:", type);
      console.log("EditContentModal - InitialData:", initialData);
      
      await onSave(dataToSave);
      console.log("EditContentModal - Save successful");
      onClose();
    } catch (error) {
      console.error("EditContentModal - Save failed:", error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl font-bold mb-1" style={{ color: "#1A1A1A" }}>Edit {getTypeLabel()}</CardTitle>
                <p className="text-sm" style={{ color: "#4A4A4A" }}>Perbarui informasi {getTypeLabel().toLowerCase()}</p>
              </div>
            </div>
            <Button
              variant="ghost"
